...
public class MultiplayerMenu : MonoBehaviour
{
	...
	private void LocalServerLocated(NetWorker.BroadcastEndpoints endpoint, NetWorker sender)
	{
		...
		console.appendLogLine("Found endpoint: " + endpoint.Address + ":" + endpoint.Port);
	}

	public void Connect(string joinAddress)
	{
		...
		if (useTCP)
		{
			...
			((TCPClient)client).Connect(joinAddress, (ushort)port);
		}
		else
		{
			client = new UDPClient();
			if (natServerHost.Trim().Length == 0)
				((UDPClient)client).Connect(joinAddress, (ushort)port);
			else
				((UDPClient)client).Connect(joinAddress, (ushort)port, natServerHost, natServerPort);
		}
