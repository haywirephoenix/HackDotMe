...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] HackerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is HackerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (HackerNetworkObject.Length > 0 && HackerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(HackerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateHacker instead, its shorter and easier to type out ;)")]
		public HackerBehavior InstantiateHackerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(HackerNetworkObject[index]);
			var netBehavior = go.GetComponent<HackerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<HackerBehavior>().networkObject = (HackerNetworkObject)obj;
		...
		public HackerBehavior InstantiateHacker(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(HackerNetworkObject[index]);
			var netBehavior = go.GetComponent<HackerBehavior>();
